...
class Twiggy
{
	...
	*/       	
	...
	public function display( $template = FALSE )
	{
		...
		$this->set('_template', $template ? $template : $this->_template);

		if ( $template )
		{
			$this->template( $this->portal_path( $template ) );
		}
		else
		{
			$this->template( $this->portal_path( $this->get_template() ) );
		}
	...
	public function portal_path( $path, $with_extension = FALSE )
	{
		if ( file_exists( APPPATH.'themes/'.$this->get_theme().'/'.$this->get_layout().'/'.$path.$this->_config['template_file_ext'] ) )
		{
			$path = $this->get_layout().'/'.$path;
		}

		return $with_extension ? $path.$this->_config['template_file_ext'] : $path;
	}

	/**
	 * Esta función es propia de las necesidades de amovens. Funciona igual que display pero en caso de que un archivo no exista lo busca en el tema por defecto.
	 *
	 * @access	public
	 * @return	void
	 */
	public function show()
	{
		...
		//Si no existe el fichero que intentamos cargar:
		if ( ! file_exists( APPPATH.'themes/'.$current_theme.'/'.$this->get_template().$this->_config['template_file_ext'] ) )
		{
			$this->theme( );
			//Si no existe lo cargamos del tema por defecto:
			$theme = 
			$this->theme( );

		}
		echo $this->get_theme().'/'.$this->get_template().$this->_config['template_file_ext'].' '.$this->_config['default_theme'].'/'.$this->get_template().$this->_config['template_file_ext'];
		exit;
	}
